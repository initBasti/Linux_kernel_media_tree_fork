...
struct rockchip_dw_dsi_chip_data {
	...
	int (*dphy_rx_init)(struct phy *);
	int (*dphy_rx_power_on)(struct phy *);
	int (*dphy_rx_power_off)(struct phy *);
...
static int dw_mipi_dsi_dphy_power_on(struct phy *phy)
{
	...
	dev_dbg(dsi->dev, "lanes %d - data_rate_mbps %u\n",
	...
	if (dsi->cdata->dphy_rx_power_on) {
		...
		if (ret < 0) {
			dev_err(dsi->dev, "hardware-specific phy shutdown failed: %d\n", ret);
			clk_disable_unprepare(dsi->phy_cfg_clk);
			clk_disable_unprepare(dsi->grf_clk);
			return ret;
		}
